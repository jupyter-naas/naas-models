...
from protobuf_to_pydantic.field_info_rule.protobuf_option_to_field_info.types import UriRefStr
...
class Container(BaseModel):
    ...
    image: typing.Optional[str] = Field(default="", min_length=1, pattern="^[a-zA-Z0-9\\.\\/-]+([:@][a-zA-Z0-9\\.\\/:_-]*)?$")
...
class SpaceCreationResponse(BaseModel):
    space: typing.Optional[Space] = Field(default_factory=Space)
    status: typing.Optional[str] = Field(default="")
...
class SpaceUpdateRequest(BaseModel):
    name: typing.Optional[str] = Field(default="", min_length=1, max_length=63, pattern="^([A-Za-z0-9]+(-[A-Za-z0-9]+)+)$")
    space: typing.Optional[SpaceUpdate] = Field(default_factory=SpaceUpdate)# optional string domain = 4 [(validate.rules).string.uri_ref = true];
